/*******************************
 * Plots custom donut chart
 * http://blog.stephenboak.com/2011/08/07/easy-as-a-pie.html
 * jsfiddle - http://jsfiddle.net/stephenboak/hYuPb/
 * @param {Object} options
 * element
 * data
 * colors
 *******************************/
customDonut = function(options) {

    var el = options.element;
    el.addClass('custom-donut');
    var w = el.width();
    var h = el.height();
    var r = Math.min(w, h) * .3;
    var ir = Math.min(w, h) * .15;
    var textOffset = 14;
    ...
    var vis = d3.select(el[0]).append("svg:svg").attr("width", w).attr("height", h);
    ...
    function update(data) {
        ...
        streakerDataAdded = _.map(data, function(xs) {
            return {
                port : xs[0],
                octetTotalCount : xs[1]
            };
        });
        ...
        if (filteredPieData.length > 0 && oldPieData.length > 0) {
            ...
            var tmpSum = _.reduce(_.pluck(filteredPieData, 'value'), function(agg, v) {
                return agg + v;
            });
            largeFilteredPieData = _.filter(filteredPieData, function(d) {
                return d.value / tmpSum > 0.07;
            });
            console.log(largeFilteredPieData);
            lines = label_group.selectAll("line").data(largeFilteredPieData);
            lines.enter().append("svg:line").attr("x1", 0).attr("x2", 0).attr("y1", -r - 3).attr("y2", -r - 8).attr("stroke", "gray").attr("transform", function(d) {
            ...
            valueLabels = label_group.selectAll("text.value").data(largeFilteredPieData).attr("dy", function(d) {
            ...
            valueLabels.attr("fill-opacity", 0.3).transition().duration(tweenDuration).attrTween("transform", textTween).attr("fill-opacity", 1);

            valueLabels.exit().remove();

            //DRAW LABELS WITH ENTITY NAMES
            nameLabels = label_group.selectAll("text.units").data(largeFilteredPieData).attr("dy", function(d) {
                if ((d.startAngle + d.endAngle) / 2 > Math.PI / 2 && (d.startAngle + d.endAngle) / 2 < Math.PI * 1.5) {
                    return 17;
                } else {
                    return 5;
            ...
            nameLabels.attr("fill-opacity", 0.3).transition().duration(tweenDuration).attrTween("transform", textTween).attr("fill-opacity", 1);
    ...
    // set first update and return function to update
    update(options.data);
    update(options.data);
    return {
        update : update
