...
(function (context, dimple) {
    ...
    dimple.chart = function (svg, data) {
         ...
         ***************************************************************************************/
        this._hoverTooltipGroup = null;
        this._sweetAreaGroup = svg.append("g");
        ...
        this.draw = function (duration, noDataChange) {
            /***************************************************************************************
             * Custom Code erase sweet area on draw
             ***************************************************************************************/
            dimple._removeTooltip(null, null, this, null);
            this._hoverTooltipGroup&&this._hoverTooltipGroup.remove();
    ...
    dimple.series = function (chart, categoryFields, xAxis, yAxis, zAxis, colorAxis, pieAxis, plotFunction, aggregateFunction, stacked) {
        /***************************************************************************************
         * Custom Code to fix drop destination bug on chart redraw
         ***************************************************************************************/
        this.dropDest = null;
    ...
    dimple.plot.bubble = {
        ...
        draw: function (chart, series, duration) {
            ...
            theseShapes
                ...
                /***************************************************************************************
                 * Custom Code for cursor pointer
                 ***************************************************************************************/
            ...
            updated = chart._handleTransition(theseShapes, duration, chart, series)
                ...
                .attr("r", function (d) {
                    ...
                    /***************************************************************************************
                     * Custom Code for radius
                     ***************************************************************************************/
                    // return as a percentage of graph height
                    return Math.max(chart._heightPixels()*.012,5);
                })
                .call(function () {
                    if (!chart.noFormats) {
                        this.attr("fill", function (d) {
    ...
    dimple._removeTooltip = function (e, shape, chart, series) {
        ...
        chart._sweetAreaGroup.selectAll("*").remove();
    };
    /*jslint unparam: false */
    ...
    dimple._showPointTooltip = function (e, shape, chart, series) {
        ...
        // Collect some facts about the highlighted bubble
             ...
             * Custom Code to fix drop destination bug on chart redraw
             ***************************************************************************************/
            dropDest = series.dropDest ? series.dropDest : (series.dropDest = series._dropLineOrigin(), series.dropDest),
        ...
        /***************************************************************************************
         * Custom Code to make sweet region
         ***************************************************************************************/
        chart._sweetAreaGroup.selectAll("*").remove();
        var rectX = (cx < dropDest.x ? dropDest.x - 1 : dropDest.x + 1);
        var rectY = dropDest.y;
        // add sweet area
        chart._sweetAreaGroup.append("rect").attr("x", rectX).attr("y", cy).attr("width", cx-rectX).attr("height", rectY-cy).style("fill", "#FFB155")
        .style("opacity", 0).transition().style("opacity", 0.4).delay(animDuration).duration(animDuration / 2).ease("linear");
        // add sweet text
        // chart._tooltipGroup.append("text").attr("class", "dimple-sweet-area").attr("x", rectX+5).attr("y",rectY-5).text("3 sweeter plans")
        // .style("font-family", series.tooltipFontFamily).style("font-size", series._getTooltipFontSize()).style("display", "none").transition()
        // .style("display", "block").delay(animDuration);
